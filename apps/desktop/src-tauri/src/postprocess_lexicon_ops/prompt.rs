...
pub fn build_stage_b_merged_proofread_prompt(
    ...
    let mut lines = vec![
        ...
        "3. 每条 op 必须带 evidence：type 为 punctuation | rule | glossary | inconsistent_term | homophone。".to_string(),
        ...
        "8. 同音误写但 Pack 中无对应规则时，用 homophone + ref「错→对」；会作为「同音推测」候选展示，由用户确认。".to_string(),
        "9. 不要 merge/split 语段；不要润色、扩写或改时间轴。".to_string(),
        "10. 无把握时返回空 ops。".to_string(),
