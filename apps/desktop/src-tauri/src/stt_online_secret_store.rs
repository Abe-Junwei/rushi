...
const SECRETS_DIR: &str = "secrets/stt-online";
const KEYRING_SERVICE: &str = "studio.lingchuang.rushi";
const KEYRING_USER_PREFIX: &str = "stt:";

pub fn stt_secret_path(app_data_root: &Path, api_key_id: &str) -> PathBuf {
    app_data_root
        .join(SECRETS_DIR)
...
fn write_file_secret(app_data_root: &Path, api_key_id: &str, api_key: &str) -> Result<(), String> {
    let dir = secrets_dir(app_data_root);
    fs::create_dir_all(&dir).map_err(|e| format!("无法创建本地密钥目录：{e}"))?;
    let path = stt_secret_path(app_data_root, api_key_id);
    fs::write(&path, api_key.as_bytes()).map_err(|e| format!("写入 STT API Key 失败：{e}"))?;
    restrict_secret_file_permissions(&path)?;
    Ok(())
}
...
fn delete_file_secret(app_data_root: &Path, api_key_id: &str) -> Result<(), String> {
    ...
    match fs::remove_file(&path) {
        Ok(()) => Ok(()),
        Err(e) if e.kind() == std::io::ErrorKind::NotFound => Ok(()),
        Err(e) => Err(format!("删除 STT API Key 文件失败：{e}")),
    }
}
...
pub fn stt_secret_exists(app_data_root: &Path, api_key_id: &str) -> Result<bool, String> {
    if stt_secret_path(app_data_root, api_key_id).is_file() {
        return Ok(true);
    }
...
pub fn read_stt_secret(app_data_root: &Path, api_key_id: &str) -> Result<Option<String>, String> {
...
pub fn delete_stt_secret(app_data_root: &Path, api_key_id: &str) -> Result<(), String> {
    if use_keyring_store() {
        let _ = delete_keyring_secret(api_key_id);
    }
...
mod tests {
    ...
    fn exists_check_uses_file_without_keyring_read() {
        ...
        let root = std::env::temp_dir().join(format!("rushi-stt-exists-{}", Uuid::new_v4()));
        write_file_secret(&root, "default", "sk-exists-only").unwrap();
        assert!(stt_secret_exists(&root, "default").unwrap());
        assert_eq!(
            read_stt_secret(&root, "default").unwrap().as_deref(),
            Some("sk-exists-only")
        );
        delete_stt_secret(&root, "default").unwrap();
        assert!(!stt_secret_exists(&root, "default").unwrap());
    ...
    fn secret_roundtrip_in_app_data_dir() {
        std::env::set_var("RUSHI_LLM_SECRET_FORCE_FILE", "1");
        let root = std::env::temp_dir().join(format!("rushi-stt-secret-{}", Uuid::new_v4()));
        write_stt_secret(&root, "default", "sk-roundtrip-test").unwrap();
        assert!(stt_secret_exists(&root, "default").unwrap());
        assert_eq!(
            read_stt_secret(&root, "default").unwrap().as_deref(),
