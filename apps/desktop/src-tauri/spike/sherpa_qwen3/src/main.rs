...
use rushi_spike_sherpa_qwen3::{recognize_wav, recognize_wav_vad};
...
struct Args {
    ...
    #[arg(long, value_enum, default_value = "whole", env = "SHERPA_QWEN3_PIPELINE")]
    pipeline: Pipeline,
...
fn run() -> Result<(), String> {
    ...
    let result = match args.pipeline {
        Pipeline::Whole => {
            recognize_wav(&args.model_dir, &args.wav, &args.provider, args.threads, hotwords)
