import { memo, useCallback, useMemo } from "react";
import {
  clampP1PxPerSec,
  P1_PX_PER_SEC_MAX,
  P1_PX_PER_SEC_MIN,
  P1_TIMELINE_PX_PER_SEC,
} from "../utils/p1PxPerSec";

export type P1WaveformZoomBarProps = {
  disabled: boolean;
  isReady: boolean;
  pxPerSec: number;
  hasSelectionSegment: boolean;
  onZoomToFitAll: () => void;
  onZoomToFitSelection: () => void;
  onZoomOneToOne: () => void;
  onZoomIn: () => void;
  onZoomOut: () => void;
  onPxPerSecChange: (pxPerSec: number) => void;
};

function pxPerSecToSliderPos(px: number): number {
  const c = clampP1PxPerSec(px);
  const lo = P1_PX_PER_SEC_MIN;
  const hi = P1_PX_PER_SEC_MAX;
  return Math.round((Math.log(c / lo) / Math.log(hi / lo)) * 1000);
}

function sliderPosToPxPerSec(pos: number): number {
  const lo = P1_PX_PER_SEC_MIN;
  const hi = P1_PX_PER_SEC_MAX;
  const t = Math.min(1000, Math.max(0, pos));
  return clampP1PxPerSec(lo * (hi / lo) ** (t / 1000));
}

function IconFitAll({ className }: { className?: string }) {
  return (
    <svg className={className} width="18" height="18" viewBox="0 0 24 24" aria-hidden fill="none">
      <path
        d="M9 3H3v6M15 3h6v6M3 15v6h6M21 15v6h-6"
        stroke="currentColor"
        strokeWidth="2"
        strokeLinecap="round"
        strokeLinejoin="round"
      />
    </svg>
  );
}

function IconFitSelection({ className }: { className?: string }) {
  return (
    <svg className={className} width="18" height="18" viewBox="0 0 24 24" aria-hidden fill="none">
      <circle cx="12" cy="12" r="3" stroke="currentColor" strokeWidth="2" />
      <path
        d="M12 5V3M12 21v-2M5 12H3M21 12h-2M6.34 6.34 4.93 4.93M19.07 19.07 17.66 17.66M17.66 6.34 19.07 4.93M4.93 19.07 6.34 17.66"
        stroke="currentColor"
        strokeWidth="1.5"
        strokeLinecap="round"
      />
    </svg>
  );
}

/** 解语 TranscriptionLayoutSections / ZoomControls 同款：底部横向缩放条（布局与 icon-btn + 滑块 + 百分比）。 */
export const P1WaveformZoomBar = memo(function P1WaveformZoomBar({
  disabled,
  isReady,
  pxPerSec,
  hasSelectionSegment,
  onZoomToFitAll,
  onZoomToFitSelection,
  onZoomOneToOne,
  onZoomIn,
  onZoomOut,
  onPxPerSecChange,
}: P1WaveformZoomBarProps) {
  const off = disabled || !isReady;
  const sliderValue = useMemo(() => pxPerSecToSliderPos(pxPerSec), [pxPerSec]);
  const zoomPercentLabel = useMemo(
    () => Math.round((pxPerSec / P1_TIMELINE_PX_PER_SEC) * 100),
    [pxPerSec],
  );

  const onSliderChange = useCallback(
    (e: React.ChangeEvent<HTMLInputElement>) => {
      const pos = Number(e.target.value);
      onPxPerSecChange(sliderPosToPxPerSec(pos));
    },
    [onPxPerSecChange],
  );

  return (
    <div className="p1-waveform-zoom-toolbar" role="toolbar" aria-label="波形时间轴缩放">
      <div className="p1-waveform-zoom-bar">
        <button
          type="button"
          className="p1-icon-btn"
          disabled={off}
          title="适配整段时长到视口"
          aria-label="适配整段"
          onClick={onZoomToFitAll}
        >
          <IconFitAll className="shrink-0 opacity-90" />
        </button>
        <button
          type="button"
          className="p1-icon-btn"
          disabled={off || !hasSelectionSegment}
          title="将当前选中语段适配到视口"
          aria-label="适配选中语段"
          onClick={onZoomToFitSelection}
        >
          <IconFitSelection className="shrink-0 opacity-90" />
        </button>
        <button type="button" className="p1-icon-btn" disabled={off} title="默认横向比例（56 px/s）" aria-label="一比一" onClick={onZoomOneToOne}>
          <span className="p1-icon-btn-label">1:1</span>
        </button>
        <span className="p1-toolbar-sep" aria-hidden />
        <button type="button" className="p1-icon-btn p1-icon-btn-compact" disabled={off} title="缩小" aria-label="缩小" onClick={onZoomOut}>
          <span className="text-base leading-none">−</span>
        </button>
        <button type="button" className="p1-icon-btn p1-icon-btn-compact" disabled={off} title="放大" aria-label="放大" onClick={onZoomIn}>
          <span className="text-base leading-none">+</span>
        </button>
        <span className="p1-toolbar-sep" aria-hidden />
        <input
          type="range"
          className="p1-waveform-zoom-slider"
          min={0}
          max={1000}
          step={1}
          disabled={off}
          value={sliderValue}
          onChange={onSliderChange}
          title={`横向比例 ${zoomPercentLabel}%（相对默认）`}
          aria-label={`横向缩放滑块，当前 ${zoomPercentLabel}%`}
        />
        <span className="p1-waveform-zoom-value" aria-live="polite">
          {zoomPercentLabel}%
        </span>
        <span className="p1-waveform-zoom-px font-mono tabular-nums text-zen-stone/80" title="像素/秒">
          {Math.round(pxPerSec)} px/s
        </span>
      </div>
    </div>
  );
});
