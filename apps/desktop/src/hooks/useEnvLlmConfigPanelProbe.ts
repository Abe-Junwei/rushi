...
export function useEnvLlmConfigPanelProbe({
  ...
  const buildProbeRuntime = useCallback(
    (opts?: { preferPersistedCredentials?: boolean }): PostprocessRuntimeBridge => {
      const typedApiKey = opts?.preferPersistedCredentials ? "" : apiKey.trim();
  ...
  const probe = useCallback(
    async (opts?: { preferPersistedCredentials?: boolean }) => {
      const startedAt = Date.now();
      setProbeBusy(true);
      try {
        const runtime = buildProbeRuntime(opts);
