#!/usr/bin/env python3
"""R3g-B Qwen3 spike: Paraformer vs Qwen3-ASR-0.6B transcribe comparison (direct engine path)."""
...
ZHikong = ROOT / "fixtures/eval/samples/制控.mp3"
...
def _run(model_id: str, audio: Path, hotwords: str | None = None) -> dict:
    ...
    t0 = time.monotonic()
    with tempfile.TemporaryDirectory(prefix="qwen3_spike_") as tmp:
...
def main() -> int:
    _apply_models_root()
    out_dir = Path(os.environ.get("SPIKE_OUT_DIR", ROOT / "docs/execution/spike-output/qwen3-2026-06-03"))
    ...
    report: dict = {"long_audio": str(long_audio), "short_audio": str(short_audio), "runs": []}

    # S4 short
    for model, name in ((PARAFORMER, "paraformer"), (QWEN, "qwen")):
        print(f"==> S4 short {name} …", flush=True)
        d = _run(model, short_audio)
        (out_dir / f"s4-short-{name}.json").write_text(
            json.dumps(d, ensure_ascii=False, indent=2), encoding="utf-8"
        )
        report["runs"].append(_summarize(f"S4-{name}", d))

    # S3 term hit (制控 + hotwords)
    if ZHikong.is_file():
        for model, name in ((PARAFORMER, "paraformer"), (QWEN, "qwen")):
            print(f"==> S3 制控 hotwords {name} …", flush=True)
            d = _run(model, ZHikong, hotwords="制控")
