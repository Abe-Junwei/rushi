...
| 语段自然结束 | scoped end | **仅 pause**（不媒体 seek、不视觉回段首）；播放头停在段尾；**保持语段会话** + 记 `autoStopped` 标记（不可见）；下次 play → 段首重播（点一次即可）。任何段尾媒体状态扰动会嵌套 WebKit RemoteAudioSession 同步 IPC 致假死，故一律不做后续动作 |
