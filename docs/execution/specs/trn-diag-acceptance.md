...
> **状态**：✅ 签收（2026-06-11）  
> **手测清单**：[`trn-diag-hand-test-checklist.md`](./trn-diag-hand-test-checklist.md)  
...
- [x] 故意在 **transcribe** 阶段失败（如断侧车）：UI 标明阶段为转写/侧车，非「未知错误」 — 手测清单 §A + 组件/控制器测试 2026-06-11
- [x] 导出诊断包含 `transcribe_timeline[]`（或等价字段）且与 UI 一致 — 手测清单 §B + `persist_and_load` 2026-06-11
- [x] 与 R3e 失败分类文案不矛盾（同一 `error_code` 映射表 · `infer_transcribe_error_code`）
- [x] `npm run typecheck && npm run test && node scripts/check-architecture-guard.mjs`（编码轮）
