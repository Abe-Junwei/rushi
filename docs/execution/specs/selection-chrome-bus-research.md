...
| 模块 | 路径 |
|------|------|
| 波形 imperative chrome | `apps/desktop/src/services/selection/applySelectionChromeImperative.ts` |
| 选中唯一内核 | [`useTranscriptionLayerSelection.ts`](../../../apps/desktop/src/pages/useTranscriptionLayerSelection.ts) |
| 列表 DOM 锚点 | `[data-seg-row]` · `.seg-row-selected` · `apps/desktop/src/components/SegmentTextListRow.tsx` |
| 列表 scroll / 虚拟窗 | `apps/desktop/src/components/editor/useEditorSegmentListScroll.ts` · [`segmentListVirtualWindowCore.ts`](../../../apps/desktop/src/utils/segmentListVirtualWindowCore.ts) |
| 多选逻辑 | `apps/desktop/src/pages/useSegmentSelectionController.ts` |
