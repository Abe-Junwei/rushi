...
3. **学习要有门槛（Descript 北向，见 MEM Plan §2.2）**：**L1 术语表** ≈ Descript **≥3 次 + 确认**（本仓 F6 弹窗 ✅）；**F1 稳定规则** ≈ **`hit≥2`**（比 Descript glossary 更早一层，仅服务**当前稿字面改**）；**自动满 3 进表** → MEM-P1 可选设置。  
...
### 4.3 中文粒度拍板（2026-05-31，**已编码**）

| 决策 | 说明 |
|------|------|
| **不**做 diff 左右「扩字符」猜词 | 无分词、易过扩；与 CSC/ASR 论文「错字干扰分词」同风险 |
| **不**自动学习 **单字 CJK** `wrong→right` | `should_learn_inferred_replacement` 拒绝；infer 仍可算出最小 diff，但不入库 |
| **显式路径** | 查找替换 `explicit_pairs`（用户给定串）；「纳入记忆」确认时 `buildConfirmExplicitPair`（≥2 字且过门槛才 upsert） |
| **单字改词** | 用查找替换（如 `晶莹→凌晶`）或术语表；勿指望 save 推断 |

### 4.4 建议增量（规划纳入 R3t-F，部分未编码）

| 优先级 | 能力 | 对标 | 说明 |
|--------|------|------|------|
| **P1** | **F2 查找替换** 写回后走 **同一 save 学习** | Sonix Correct All + 书图标 | Replace All 确认后须触发 save/学习路径（**explicit_pairs 已接线**） |
| **P1** | **F6** 第三次命中提示 **加入 glossary** | Descript 自动 glossary | ✅ 已编码（`hit≥3` 弹窗）；**自动进表可关** → MEM-P1 |
...
| 2026-05-31 | §4.3：中文记忆粒度拍板（拒单字自动学、撤字符扩展、显式对优先） |
