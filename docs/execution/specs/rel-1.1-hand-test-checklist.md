...
| ID | 步骤 | 预期 |
|----|------|------|
| H-BATCH-1a | 打开有文件的项目 Hub →「导入音频」多选 **≥3** 个 mp3 | 列表出现 3+ 文件；无多次全页闪烁 |
| H-BATCH-1b | 对 **≥2** 个空语段音频点「批量转写」 | 队列串行；完成后均有语段 |
| H-BATCH-1c | 其中一个文件已有语段再批量 | 该文件 **跳过**；其余正常 |
| H-BATCH-1d | 批量进行中尝试关窗或换项目 | 被阻塞（与单文件转写一致） |
