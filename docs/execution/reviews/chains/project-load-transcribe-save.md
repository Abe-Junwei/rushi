...
| 4 | current + currentFileId | 编辑语段（`useSegmentDraft`） | `segments` 内存更新（blur 或 flush 时提交） | OK |
| 5 | 有未 flush 草稿 | `saveSegments` | `flushSegmentTextDrafts` → `file_save_segments`（按 uid upsert）→ reload | OK |
