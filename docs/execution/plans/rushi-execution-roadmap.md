...
| 最近对照 | **2026-06-17**：R3g-C **Fun-ASR-Nano** PyTorch spike **Defer**；vLLM 路线 research ✅；v1.1 主轨 Step 6–9 ✅ · **BATCH-TXN** Step 10–11 待编码 |
...
| **Q-ASR-1** | **SenseVoice** = **兼容/快轨遗留**；**v1 主推 Paraformer**；**Qwen3-ASR ❌ No-go**（2026-06-03）；**Fun-ASR-Nano PyTorch ❌ Defer**（2026-06-17，不上 catalog）；重开第三 SKU 须 **R3g-B-Align Go** 或 **Nano vLLM CUDA spike Go** |
...
| — | R3h-E/F、R3g-B | 延后 / P1 spike | — | 高级 pip/本地构建；**Qwen3 SKU** ❌ No-go；**Fun-ASR-Nano PyTorch** ❌ Defer；**Nano+vLLM** research ✅（待 CUDA spike）；索引 [`r3-asr-landscape-2026-05-improvement-backlog.md`](../specs/r3-asr-landscape-2026-05-improvement-backlog.md) §4.1.8 | remediation §5 Phase 5、[`r3g-c-funasr-nano-spike-results.md`](../specs/r3g-c-funasr-nano-spike-results.md)、[`r3g-c-funasr-nano-vllm-research.md`](../specs/r3g-c-funasr-nano-vllm-research.md) |
...
| **Qwen3 门控** | **⑤″f-E** ✅ 已签收 **No-go**（2026-06-03）；**EXP-WORD 不再等待**第三 SKU；重开须 **R3g-B-Align** Go |
...
| **P2** | **R3g-B-Align** | Qwen3 **ForcedAligner** 时间轴 spike（`forced_aligner` + R3t-A 契约）；**Go 后才可重开 R3g-B catalog** | **2–4d** | ⑤″f-E No-go | 📋 [research](../specs/r3g-b-align-qwen3-forced-aligner-spike-research.md) ✅ |
...
| **P2** | **R3g-C-NANO vLLM** | GPU 第二运行时（`AutoModelVLLM`） | spike 2–4d | PyTorch Defer + **CUDA 环境** | 📋 research ✅ — [`vllm-research`](../specs/r3g-c-funasr-nano-vllm-research.md) |
...
**建议叠加顺序**（backlog §5）：（主序）v1 **已闭合** → **§10.4 v1.1+** Step 10–12；**⑤″f-E** ❌ No-go；**R3g-C-NANO PyTorch** ❌ Defer（dev venv 已试 GitHub `main` @ `faffb45a`，未改 bundled lock）；**R3g-B-Align** / **Nano vLLM CUDA spike** **不挡 v1.1**；并行 **R3e-C.5**、**R3h-ASR-VER**（lock bump 待 vLLM/Nano 决策后）。
...
| **当前主刀** | **Step 10–11 BATCH-TXN**（Hub 批量导入 + 串行转写队列）→ **Step 12 REL-1.1**；**Step 5–9 ✅**（含 CSP / STT 取消 / DELIV / ONBOARD） |
...
| **R3g-C-NANO vLLM** | 📋 research ✅；**CUDA spike 未执行**（当前开发机 macOS） |
...
> **⚠️ 2026-06-12**：§10.4 spec 立项进度 — **CSP-HARDEN 已立项**（`csp-harden-v1.1-{research,intent,plan,acceptance}.md`）；其余五组（`product-metadata-v1.1-*` 等）仍未立项 — **禁止**无 spec 开业务编码。
...
| **热点门禁** | guard **≥46** 且持续叠功能前，优先 T-010 拆 `run_transcribe_cmd.rs` / `online_segment_normalize.rs` |
...
## 13. 代码对照评估（2026-06-12，`main` / 工作区）

> 对照 **已推送 `main` 或工作区等价**。发版轮末刷新本节测试数 / 守卫警告 / 热点行数。

### 13.1 工程验证快照（实测 2026-06-12）

| 检查项 | 结果 |
|--------|------|
| `npm run typecheck` | ✅ 通过 |
| `npm run test`（desktop） | ✅ **1273** passed（**257** files） |
| `cargo test --lib`（desktop） | ✅ **348** passed |
| `node scripts/check-architecture-guard.mjs` | ✅ 0 错误，**46 警告**（§13.3；TS >300 行 ~35 处；hooks >12：**4** 处） |
...
### 13.3 代码热点（2026-06-12 · guard **46** 警告）

> **分布**：TS/TSX **>300 行 ~35 处**；**hooks >12：4 处**；Rust/Python 大模块见下表。Epic 签收后刷新本表。

| 文件 | 行数 / 备注 | 路线图 / 守卫 | 判定 |
|------|-------------|---------------|------|
| `online_segment_normalize.rs` | **811** 行 | 在线 STT | ⚠️ **T-010 优先拆** |
| `run_transcribe_cmd.rs` | **675** 行 | 转写入口 | ⚠️ **T-010 优先拆** |
| `model_prepare.py` | **487** 行 | R3g prepare | ⚠️ 建议拆模块 |
| `editorShortcutRegistry.ts` | **498** 行 | 快捷键 | ⚠️ 建议拆分 |
| `useTranscriptionLayer.ts` | **423** 行 | 编辑器层 | ⚠️ 超 300 |
| `useTranscribeJobController.ts` | **379** 行 · **14 hooks** | R3e-C + STT | ⚠️ 超 300 / 超 12 hooks |
| `postTranscribeStageB.ts` | **383** 行 | F0 阶段 B | ⚠️ 接近上限 |
| `pxPerSec.ts` / `.test.ts` | **376** / **360** | 波形 | ⚠️ |
| `local_runtime/install_support/`（合计） | 仍大 | R3h-I2 | ⚠️ 随 LRC 薄片拆 |
| `useAsrSetupController.ts` | ~122 | R3h-I3 前端 | ✅ 已拆分；Rust Setup FSM 🟡 |
| `dialogEscapeStack.ts` | 新 | 浮动 dialog Esc | ✅ |

### 13.4 排期调整摘要（2026-05-30）

1. **R3t-A/B/C**、**R3e-B/C ✅**（C 2026-05-31）；§4.1.1 **⑥½** 闭合。  
2. **§4.1.8** ASR 生态 backlog；P1 可并行 R3h-ASR-VER / Qwen3 spike。  
3. **下一编码刀**：**§10.4 Step 10–11 BATCH-TXN** → **REL-1.1**；并行 **R3g-C-NANO vLLM**（仅 CUDA 环境）。
...
| **v1.1 spec 未立项** | **中** | §10.4 六组 `*-intent/plan/acceptance` **缺失** — 编码前必补 |
| **guard 46 警告** | **中** | Step 8 前 **T-010** 拆 `run_transcribe_cmd` / `online_segment_normalize` |
