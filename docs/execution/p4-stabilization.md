...
- 目录：[`fixtures/p4-eval/`](../../fixtures/p4-eval/README.md)
- 清单：`eval_manifest.v1.json`（`schema_version`、`items[]`）
- 占位音频：`bash scripts/p4-eval-generate-placeholders.sh`（需 ffmpeg）
...
**CI**：`asr` job 在 `pytest` 后 **后台启动 stub ASR**（`python -m rushi_asr`），待 `GET /health` 就绪后执行 `python3 scripts/p4-eval-run.py`（仓库根 manifest + 占位 wav）。

**批跑清单（需本机 ASR + curl）**：仓库根执行 `npm run p4:eval-run`（或 `python3 scripts/p4-eval-run.py`）。会读取 `fixtures/p4-eval/eval_manifest.v1.json`，对每条 `audio_relpath` 调用 `POST /v1/transcribe`，在 stdout 打印 JSON（含 `engine`、`warnings`、拼接假设文本、`low_confidence_ratio`、有参考稿时的 `cer_chars`）。若音频缺失或请求失败，对应 `item` 带 `error` 且进程退出码为 1。
...
示例：[`fixtures/p4-eval/checkpoint.example.json`](../../fixtures/p4-eval/checkpoint.example.json)。
...
桌面壳命令 **`p4_export_diagnostic_bundle`**：用户选择保存路径后，生成 **zip**，主要包括：
