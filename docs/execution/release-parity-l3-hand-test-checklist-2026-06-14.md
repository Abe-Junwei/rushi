...
| 日期 | 2026-06-20 |
| 测试人 | junwei |
| App 路径 | `/Volumes/如是我闻 1/如是我闻.app`（`如是我闻_0.1.1_aarch64.dmg` 挂载副本 · unsigned） |
| App 版本 | 0.1.1 |
| Git SHA（关于页 / stamp） | dc9f372 |
| macOS 版本 | 26.5.1 arm64 |
...
| # | 操作 | 期望 | ☐ |
|---|------|------|---|
| B10 | 转写后 Stage B 对话框 | 能打开；文案无 dev 命令 | N/A |
| B11 | LLM 相关 gate | 无 `npm run desktop:dev` | N/A |
...
| # | 场景 | UI 应显示 | 不应显示 | ☐ |
|---|------|-----------|----------|---|
| S1 | 冷启动 release，模型未准备 | chip 黄/红；blockReason 可操作 | `npm run …` | N/A（upgrade · 模型已缓存） |
| S2 | D1=Paraformer，D2=SenseVoice（不一致） | 「应用并重启侧车」类提示 | 「100% 已就绪」误报 | N/A |
| S3 | 一键准备完成 | chip 绿；与所选模型可转写一致 | 全局 ready 误报 | ✅ |
| S4 | ffmpeg 异常（若可模拟） | packaged ffmpeg 修复文案 | 误导「单独装 ffmpeg」 | N/A |
| S5 | 8741 被 dev 侧车占用 + 开 release app | stale/adopt 说明 | 「bundled 正常」误导 | N/A |
| S6 | 热词超 12k（若有项目） | 截断/toast 有说明 | 静默截断 | N/A |
...
| F2 | 有 Key 时在线转写（若可测） | 与 adapter 行为一致 | N/A |
| F3 | 转写进行中尝试离开编辑器 | gate 文案合理 | ✅ |
| F4 | Ollama 未启动（本机 LLM） | LLM chip + 后处理提示可操作 | ✅ |
| F5 | Stage B Consent（若启用） | 「发送云端 LLM」与字段一致 | N/A |
...
| # | 操作 | 期望 | ☐ |
|---|------|------|---|
| I1 | Editor → **查找替换** → 搜 2 处匹配 | 壳层贴内容；列表无内嵌大空白；底栏完整 | ⚠️→🔧 2026-06-20 修 flex-1/h-full；待 v0.1.2 复测 |
...
| I5 | 查找替换：手动拖高 → 改变匹配数 → **双击标题栏** | 壳高随内容重算（Restore auto height） | 未测 |
...
| # | 操作 | 期望 | ☐ |
|---|------|------|---|
| H1 | 空 App Data 首次启动 | Welcome 正常 | ☐ 未测（O1 本轮仅 upgrade） |
...
|------|----------------|---------------------------|
| A ASR chip | 4/4 | PASS |
| B 主路径 | 9/9 · 2 N/A | PASS |
| C WKWebView | 7/7 | PASS |
| D 7-A | 8/8 | PASS |
| E 7-B | 1/1 · 5 N/A | PASS |
| F 7-C | 3/3 · 2 N/A | PASS |
| G 7-D | 3/3 | PASS |
| I FLOAT-FIT | 3/4 · 1 ⚠️ · 1 未测 | WARN |
| H Fresh（可选） | 0/4 | 未测 |

**已知问题 / WARN**（可贴截图路径或 `desktop.log` 关键词）：

```
I1 FindReplaceDialog：⌘F 打开时壳层默认接近 maxHeight（FLOAT-FIT）。
→ 2026-06-20 已修：DraggableResizablePanel / FloatingPanelDialogRoot fitToContent；FindReplace layoutRev=5。
待 release 复测 I1/I5。
```
...
**结论**：☑ **L3 Go**（upgrade profile · v0.1.1 unsigned DMG） ☐ **No-Go**（列 blocker）

> Fresh H1–H4 仍 ☐；对外「新用户零终端」分发前须补跑 `r3f-fresh-appdata-hand-test.sh --interactive`。
