...
7. **保音高变速（2026-07-19）**：1.0x 仍走线性重采样；非 1.0x 经 vendored Signalsmith Stretch（`native_audio/signalsmith_tempo.rs` + C ABI wrapper）保音高；轻量 SOLA/WSOLA `native_audio/tempo.rs` 保留为对照测试基线。见 [`native-audio-pitch-preserving-rate-research.md`](../execution/specs/native-audio-pitch-preserving-rate-research.md) 与 [`native-audio-signalsmith-stretch-research.md`](../execution/specs/native-audio-signalsmith-stretch-research.md)。**仍不做**：输出设备选择 UI、WebAudio 默认、HTML keepalive、私有 WKPreference、音乐级无伪影承诺。
