...
## 附录 A — R3h-3.5 Spike 预期（2026-05-30 评估吸收）

> **背景**：FunASR PyInstaller 侧车 ~2.5GB；Sherpa-ONNX SenseVoice INT8 约 ~200MB、RTF ~0.05（2026-05 行业数据）。FireRedASR2 等最新中文 SOTA 仍主要在 PyTorch/FunASR 生态，Sherpa 未必能承载全部新权重。

### Spike 成功后的产品形态（修订预期）

| 模式 | 引擎 | 体积（约） | 定位 |
|------|------|------------|------|
| **高精度模式**（默认 v1） | FunASR 侧车 + Paraformer/Qwen3 SKU | ~2.5GB 运行时 + 权重 | 长音频多语段、专名、R3t-A 分段真源 |
| **轻量模式**（Spike Go 后可选） | Sherpa-ONNX SenseVoice INT8 等 | ~200MB 级 | 短音频、低 RAM、快速试听；**不承诺** 替代 Paraformer 长音频质量 |

### Spike 结论分支

1. **Go（轻量模式）**：另开 ADR 修订或附录 B，定义 `asr_engine` 双轨 + 环境页模式切换；**不删除** FunASR 主路径。
2. **Partial Go**：仅 Win/mac 某平台或仅 SenseVoice 路径 → 文档化平台矩阵，v1 仍可只 ship FunASR。
