...
## 附录 A — R3h-3.5 Spike 预期（2026-06-10 修订：Paraformer 对齐 catalog）

> **背景**：FunASR PyInstaller 侧车 ~2.5GB。本仓 **v1 主推 SKU 已为 Paraformer 长音频**；`SenseVoiceSmall` **已弃用**（catalog 迁移）；Qwen3-ASR **No-go**（Q-ASR-1）。Sherpa Spike **以 Paraformer ONNX 为主模型**，不再以 SenseVoice INT8 为 Go 门控。  
> **Spec**：[`r3h-3.5-sherpa-spike-research.md`](../execution/specs/r3h-3.5-sherpa-spike-research.md)
...
| P1 | `sherpa-onnx-paraformer-zh-int8-2025-10-07` | 轻量 / RTF / 体积 |
...
| 模式 | 引擎 | 体积（约） | 定位 |
|------|------|------------|------|
| **高精度模式**（默认 v1） | FunASR 侧车 + **Paraformer** SKU | ~2.5GB 运行时 + 权重 | 长音频多语段、hotwords、R3t-A 分段真源 |
| **轻量模式**（Spike Go 后可选） | Sherpa-ONNX **Paraformer INT8** 等 | ~100–300MB 级 | 低 RAM、快速试听；**不承诺** 等同 FunASR vad-punc 长音频质量 |

### Spike 结论分支

1. **Go（轻量/中期候选）**：另开 ADR 附录 B，定义 `asr_engine` 双轨；**默认仍 FunASR Paraformer**；**不删除** 侧车主路径。
2. **Partial Go**：仅某平台 / 仅短音频 / 无 hotwords → 文档化平台矩阵；v1 仍可只 ship FunASR。
3. **No-go**：在 [`asr-sidecar-funasr-policy.md`](../architecture/asr-sidecar-funasr-policy.md) 明示 ~2.5GB 为 v1 长期约束；继续 R3h-I + PyInstaller 加固。

### Spike 结论（2026-06-11）

**采纳 Partial Go** — 详见 [ADR-0006](./0006-sherpa-onnx-paraformer-spike-evaluation.md)：RTF/体积可行，长音频质量与 VAD/punc/hotwords 未达标；**v1 默认仍 FunASR Paraformer**；Sherpa harness 保留作 R&D，不接主转写路径。

### 与 FireRedASR2 的关系

- **v1 不接入** FireRedASR2（非 FunASR 生态、AED 60s 输入限制等）。
- 若 Sherpa Spike **Go**，v1 后评估 FireRedASR2-AED 是否可作为 **第三引擎** spike（见 [`r3-asr-landscape-2026-05-improvement-backlog.md`](../execution/specs/r3-asr-landscape-2026-05-improvement-backlog.md) **ASR-RADAR-FireRed**）。
