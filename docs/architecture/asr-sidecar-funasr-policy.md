...
| 连接失败且 `lsof -i :8741` 为空 | **侧车进程未运行**；`npm run asr:dev` 或 `npm run desktop:dev`。release 下也可能是 **ASR-WARM idle-stop**（默认空闲约 15 分钟回收 managed 侧车）：UI 须显示「已休眠」而非「环境异常」；「重试检测」/转写可 soft-wake（`try_start`），见 [`asr-idle-stop-ux-acceptance.md`](../execution/specs/asr-idle-stop-ux-acceptance.md)。 |
