...
- **Tauri `p1` 拉取语段**：将 SQLite 术语表拼成空格串，随 `multipart/form-data` 字段 **`hotwords`** 一并 `POST /v1/transcribe`（见 `apps/desktop/src-tauri/src/p1.rs`）。
...
| **FunASR 就绪，热词超长** | 传入前截断至 12k 字符 | `hotwords_truncated_12k` |
