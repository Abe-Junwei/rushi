...
def _prepare_result_from_cache(resolved_model_id: str, warnings: list[str]) -> dict[str, Any] | None:
    """When all required weights exist locally, skip network download entirely."""
    if not required_models_cached_guess(resolved_model_id):
        return None
    model_dir = _cached_hub_path(resolved_model_id)
    ...
    vad_dir = _cached_hub_path(vad_model_id, required_files=DEFAULT_VAD_REQUIRED_FILES, min_weight_bytes=1 * 1024 * 1024) if vad_model_id else None
    punc_dir = _cached_hub_path(punc_model_id, required_files=DEFAULT_PUNC_REQUIRED_FILES, min_weight_bytes=1 * 1024 * 1024) if punc_model_id else None
    aligner_dir = _cached_hub_path(forced_aligner_id) if forced_aligner_id else None
...
def _resolve_model_dir(
    ...
    *,
    ...
    """Return cached hub dir or download via ModelScope."""
    req_files, weight_file, min_weight = recognizer_cache_spec(model_id)
    cached = find_cached_model_dir(
        model_id,
        req_files,
        min_weight,
...
def _download_models(resolved_model_id: str) -> dict[str, Any]:
    ...
    with runtime_lock():
        ...
        cached_result = _prepare_result_from_cache(resolved_model_id, warnings)
        ...
        try:
            ...
            if vad_model_id:
                ...
                raise_if_prepare_cancelled()
                vad_dir = _resolve_aux_model_dir(
                    vad_model_id,
                    DEFAULT_VAD_REQUIRED_FILES,
                    1 * 1024 * 1024,
                    snapshot_download=snapshot_download,
            ...
            if punc_model_id:
                ...
                raise_if_prepare_cancelled()
                punc_dir = _resolve_aux_model_dir(
                    punc_model_id,
                    DEFAULT_PUNC_REQUIRED_FILES,
                    1 * 1024 * 1024,
                    snapshot_download=snapshot_download,
            ...
            if forced_aligner_id:
                _set_prepare_message("downloading_forced_aligner")
                raise_if_prepare_cancelled()
                aligner_dir = _resolve_model_dir(
                    forced_aligner_id,
                    snapshot_download=snapshot_download,
                    progress_callbacks=progress_callbacks,
                )
