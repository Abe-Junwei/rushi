...
`tauri-plugin-updater` + Ed25519；**安装包与 OTA 只走 Cloudflare R2**（`https://updates.rushi.app/`，见 [`rel-mac-ota-cdn-r2.md`](docs/execution/specs/rel-mac-ota-cdn-r2.md)）。**不**再上传 GitHub Release 资产。验签用 `TAURI_SIGNING_PRIVATE_KEY` 公钥，**不**依赖 Apple 证书。
_Avoid_: 自动更新（未说明 Tauri updater 时）, App Store 更新, 把安装包传到 GitHub Releases
