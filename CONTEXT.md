...
`tauri-plugin-updater` + Ed25519 更新 manifest；**OTA 清单与包体主站**为 Cloudflare R2 CDN（`https://updates.rushi.app/latest.json`，见 [`rel-mac-ota-cdn-r2.md`](docs/execution/specs/rel-mac-ota-cdn-r2.md)）；GitHub Release 镜像清单并继续承载 `.dmg`。验签用 `TAURI_SIGNING_PRIVATE_KEY` 公钥，**不**依赖 Apple 证书。
