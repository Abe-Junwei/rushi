...
#### Rushi 桌面 P1 项目坞（落码映射）

Clay 营销站组件在 YAML 中已定义 `button-primary` / `button-secondary` / `text-input` 的尺寸与字重，此前 **未在 YAML 写明边框**：次按钮与输入框均为 **1px `{colors.hairline}`**（已在上方 `components` 补全 `border` 字段）。如是我闻 **P1 项目侧栏、顶栏、欢迎页、环境与 ASR、确认创建** 等控件与下表及 `apps/desktop/src/config/p1ControlStyles.ts` 保持一致即可与 Clay 气质对齐：

| DESIGN | 要点 | 落码约定（Tailwind） |
|--------|------|----------------------|
| `{rounded.md}` = **12px** | 标准按钮与输入框圆角 | 使用 **`rounded-xl`**（默认 `rounded-lg` 在 Tailwind 中为 8px，易与稿不符） |
| `button-*` height **44px** | WCAG 触摸目标 | **`h-11 min-h-[44px]`** + `items-center`；水平 **`px-5`**（20px）、垂直由内边距与行高撑满 |
| `{typography.button}` | Inter **14px / 600** | **`text-sm font-semibold`** |
| `button-secondary` | 底 **`{colors.canvas}`** + hairline | **`bg-app-bg`**、`**border-zen-gray-300**`（#e5e5e5 = hairline）；hover 用 **`hover:bg-zen-ochre`**（`surface-soft`） |
| `text-input` | **`{typography.body-md}`** 16px | **`text-base font-normal`**、`**px-4 py-3**`、其余与按钮同高同圆角 |
| **Elevation** 表 | 默认 **无阴影** | 控件区 **不使用** `shadow-sm` / `shadow-md` 做常态层次，仅靠画布 / surface / hairline 对比 |
| **box-shadow** | WebKit / `ring-*` 与边框叠层 | `.p1-workspace` 内对 `input`/`textarea`/`select` 强制 **`box-shadow: none`**；聚焦用 **`outline`**，不用 `ring`；自动填充仍靠 `.p1-confirm-project-input:-webkit-autofill` 的 inset 遮罩（`!important`） |

原生 **`select`** 保留系统下拉箭头（不做全局 `appearance: none`），但 **高度、圆角、边框、背景** 与 `text-input` 对齐。
